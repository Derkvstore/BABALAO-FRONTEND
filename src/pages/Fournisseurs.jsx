  ...
  return (
    <div className="p-4 sm:p-8 max-w-full mx-auto font-sans dark:bg-gray-900 dark:text-gray-100">
      ...
      <h1 className="text-2xl sm:text-3xl font-semibold text-blue-700 dark:text-blue-400 mb-6 flex items-center">
        <BuildingOfficeIcon className="h-7 w-7 sm:h-8 sm:w-8 text-blue-600 dark:text-blue-400 mr-2" />
      ...
      {loading ? (
        <p className="text-gray-500 dark:text-gray-400 text-center mt-8">Chargement des fournisseurs...</p>
      ) : fournisseurs.length === 0 ? (
        <p className="text-gray-500 dark:text-gray-400 text-center mt-8">Aucun fournisseur trouvé.</p>
      ) : (
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {fournisseurs.map((f) => (
                <tr key={f.id} className="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150">
                  <td className="px-6 py-4 font-medium text-gray-900 dark:text-gray-100 whitespace-nowrap">{f.nom}</td>
                  <td className="px-6 py-4 text-gray-700 dark:text-gray-300 whitespace-nowrap">{f.telephone.replace(/(\d{2})(?=\d)/g, '$1 ')}</td>
                  <td className="px-6 py-4 text-gray-700 dark:text-gray-300 whitespace-nowrap">{f.adresse || "N/A"}</td>
